			...
			int num_actors,	struct thermal_actor *thermal_actor)
{
	struct thermal_cooling_device *cdev;
	int i;
	const char *ispfreq_name = "thermal-isp-";
	const char *gpufreq_name = "thermal-gpufreq-";
	...
	for (i = 0; i < num_actors; i++) {
		...
		if (strstarts(cdev->type, cpufreq_name))
			type = CPUFREQ_CDEV;
		else if (strstarts(cdev->type, gpufreq_name))
			type = GPUFREQ_CDEV;
		else if (strstarts(cdev->type, ispfreq_name))
			type = ISPFREQ_CDEV;
		else
			type = OTHER_CDEV;
			  ...
			  int control_temp)
{
	...
	memset(thermal_actor, 0, num_actors * sizeof(struct thermal_actor));
